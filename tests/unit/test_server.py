...
async def test_call_tool_current_price():
    ...
    with patch('robin_stocks_mcp.server.market_service') as mock_service:
        ...
        result = await call_tool("robinhood.market.current_price", {"symbols": ["AAPL"]})
...
async def test_call_tool_price_history():
    ...
    with patch('robin_stocks_mcp.server.market_service') as mock_service:
        ...
        result = await call_tool("robinhood.market.price_history", {
            "symbol": "AAPL",
            "interval": "day",
            "span": "year"
        })
...
async def test_call_tool_handles_invalid_argument_error():
    ...
    with patch('robin_stocks_mcp.server.market_service') as mock_service:
        mock_service.get_current_price.side_effect = InvalidArgumentError("Invalid argument")
